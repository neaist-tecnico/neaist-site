...
try {
    ...
    $matches = fetch_all('SELECT id, group_name AS `group`, round_label AS round, DATE_FORMAT(match_date, "%Y-%m-%d") AS date, TIME_FORMAT(match_time, "%H:%i") AS time, venue, home_team_id AS homeTeamId, away_team_id AS awayTeamId, home_score AS homeScore, away_score AS awayScore, status, notes, player_of_match_player_id AS playerOfMatchPlayerId, player_of_match_name AS playerOfMatchName FROM torneio_matches ORDER BY match_date IS NULL, match_date, match_time, sort_order, id');
    $players = fetch_all('SELECT id, team_id AS teamId, name, shirt_number AS shirtNumber, position, active, sort_order AS sortOrder FROM torneio_players WHERE active = 1 ORDER BY team_id, sort_order, shirt_number, name');
    ...
    json_response([
        ...
        'players' => normalize_booleans($players, ['active']),
        'goals' => normalize_booleans($goals, ['ownGoal', 'penalty']),
